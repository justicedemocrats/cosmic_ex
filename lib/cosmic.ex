defmodule Cosmic do
  ...
  def fetch_all do
    try do
      %{body: %{"bucket" => %{"objects" => objects }}} = Cosmic.Api.get("")
      ...
      |> MapSet.new
      |> Enum.each(curry_cache_type_slugs(objects))

      Stash.persist(:cosmic_cache, "./cosmic_cache")
      Logger.info "Fetched cosmic data on #{DateTime.utc_now() |> DateTime.to_iso8601()}"
    rescue
      _e in MatchError ->
  ...
  def curry_cache_type_slugs(objects) do
    fn type ->
      matches =
        objects
